...
from phase1.prompts_gpt import *
...
def one_task_procedure(curr_chat_history: List[ChatMessage],
                        ...
                        perform_evaluation: bool = True,
                        model: str = "gpt_model",  # which model to use for task completion.
                        ):
    ...
    print(f"start task {task_idx} procedure")
    ...
    task_completion_response = global_models[model](curr_chat_history)
    # append the response to chat history.
    curr_chat_history.append(task_completion_response)
    # evaluate using task_eval_prompt_chain.
    task_eval_res = task_eval_prompt_chain(
        {
            "task_ground_truth": task[ANSWER_KEY],
            "generated_response": task_completion_response.content,
        })
    ...
    print(f"end task {task_idx} procedure")
...
def personality_evaluation_procedure(curr_chat_history: List[ChatMessage], output_writer: jsonlines.Writer = None):
    ...
    print("start personality evaluation procedure")
    ...
    for category in property_evaluation_data.keys():
        ...
        for index in range(len(property_evaluation_data[category][first_key])):
            ...
            # now call API. 
            evaluation_result = global_models["gpt_model"](curr_chat_history)
            ...
            evaluation_result_jsonl = {
                "question": question,
                "answer": evaluation_result.content,
                "category": category,
                "sample_index": index,
            }
    ...
    # close the writer.
    output_writer.close()
    print("end personality evaluation procedure")
...
def main_procedure(run_id: int = 1, persona_limit=2): # 1 is for debugging
    """
    ...
    result_output_path = os.path.join("outputs_phase1_gpt", f"{run_id}")
...
if __name__ == "__main__":
    ...
    for tasks in os.listdir(os.path.join("phase1", TASK_DIR)):
        if tasks.endswith(".jsonl"):
            ...
            print(run_id)
            main_procedure(run_id=run_id, persona_limit=args.persona_limit)